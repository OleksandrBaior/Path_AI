...
describe("EA-1", () => {
  it("Verify the creating Contact Centre Manager and validation fields", () => {
    ...
    sign_up.form
      .first_name_inp()
      .should("have.css", "border-color", constants.styleElement.errorBorder);

    sign_up.form.first_name_inp().type(firstName);
    sign_up.form.first_name_inp().should("have.value", firstName);
    sign_up.form
      .first_name_inp()
      .should("have.css", "border-color", constants.styleElement.activeBorder);
    ...
    sign_up.form
      .password_inp()
      .should("have.value", constants.validPassword[0]);

    sign_up.form.password_confirm_inp().type(constants.validPassword[1]);
    sign_up.form
      .password_confirm_inp()
      .should("have.value", constants.validPassword[1]);

    sign_up.form.sign_up_btn().click();
    sign_up.form
      .password_error()
      .should("contain", constants.errorMessages.passwordError);
    sign_up.form
      .password_inp()
      .should(
        "have.css",
        "border-color",
        constants.styleElement.errorRedBorder,
      );
